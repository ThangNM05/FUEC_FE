...
export default function ImportPreviewModal({
  ...
  const allNewIndices = useMemo(() => {
    ...
    previewData.questions.forEach((q, i) => {
      if (!q.isDuplicate) s.add(i);
    });
    return s;
  }, [previewData]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {total} questions parsed · {duplicateCount} duplicate
                {duplicateCount !== 1 ? 's' : ''} · {selectedCount} selected
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-100">
          <div className="flex items-center gap-4 text-xs text-gray-600">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-white border border-gray-300 inline-block" /> New
              question
            </span>
            <span className="flex items-center gap-1.5">
            ...
            return (
              <div
                ...
                className={`flex items-start gap-3 px-5 py-3.5 transition-colors ${
                  isDup ? 'bg-red-50 cursor-not-allowed' : 'hover:bg-orange-50/50 cursor-pointer'
                }`}
              >
                {/* Checkbox */}
                <div
                ...
                >
                  ...
                  {isDup && <X className="w-3 h-3 text-red-400" strokeWidth={3} />}
                </div>

                {/* Content */}
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-medium leading-snug ${isDup ? 'text-red-600' : 'text-[#0A1B3C]'}`}
                  >
                    {q.questionContent}
                    {isDup && (
                      <span className="ml-2 text-xs font-normal text-red-400 italic">
                        (duplicate)
                      </span>
                    )}
                  </p>
